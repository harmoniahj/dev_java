...
public class SeatTestView extends JFrame implements ItemListener{
	int inwon = 0; // 선택한 좌석 수
	int max_selection = 1; // 좌석 최대 선택가능 수 
    ...
    // 좌석에 좌석 번호 붙이기 , jp_center에 좌석추가 , 좌석에 아이템 리스너추가
    	for(int i=0;i<jcb_seat.length;i++) { 
    	 ...
    	 // insert here 이벤트 핸들러와 이벤트 소스 매핑
    		jcb_seat[i].addItemListener((ItemListener) stvEvent);
    	} 
